...
Examples:
  lightning iam user create alice
  lightning iam user create bob --role admin --scope auth.user.read`,
	Args: cobra.ExactArgs(1),
	RunE: runUserCreate,
}
...
Examples:
  lightning iam user update <id> --password
  lightning iam user update <id> --role admin --role viewer
  lightning iam user update <id> --enabled=false`,
	Args: cobra.ExactArgs(1),
	RunE: runUserUpdate,
}
...
Examples:
  lightning iam role create editor --description "Can edit content"
  lightning iam role create admin --include viewer --scope auth.user.read`,
	Args: cobra.ExactArgs(1),
	RunE: runRoleCreate,
}
...
Examples:
  lightning iam role update <id> --description "New description"
  lightning iam role update <id> --scope auth.user.read --scope auth.role.read`,
	Args: cobra.ExactArgs(1),
	RunE: runRoleUpdate,
}
...
Examples:
  lightning iam token create ci-token
  lightning iam token create admin-token --scope auth.user.read --scope auth.role.read
  lightning iam token create temp-token --expires 2024-12-31T23:59:59Z`,
	Args: cobra.ExactArgs(1),
	RunE: runTokenCreate,
}
