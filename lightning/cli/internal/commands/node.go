...
node commands. Use 'lightning node context match' to set the match context
...
from the control plane and stores it for use with all 'lightning node' commands.

Use --container-id and --match-id for commands that operate on specific containers.

Examples:
  lightning node context set engine-node-1
  lightning node context set engine-node-1 --container-id 1 --match-id node-1-1-1`,
...
for use with commands like 'lightning node tick', 'lightning command', 'lightning snapshot'.
...
Examples:
  lightning node tick advance           # Advance by 1 tick
  lightning node tick advance -n 10     # Advance by 10 ticks`,
...
func runNodeTickAdvance(cmd *cobra.Command, args []string) error {
	...
	if config.GetCurrentNodeID() == "" {
		return fmt.Errorf("no context set. Use 'lightning node context set <node-id>' first")
	}
...
Examples:
  ...
  lightning node tick get -o json`,
	RunE: runNodeTickGet,
}
...
Examples:
  lightning node simulation play                    # Default 16ms (~60 FPS)
  lightning node simulation play --interval-ms 33   # ~30 FPS
  lightning node sim play -i 8                      # ~120 FPS`,
	RunE: runNodeSimPlay,
}
...
Examples:
  lightning node metrics container
  lightning node metrics container -o json`,
...
Examples:
  lightning node metrics get
  lightning node metrics get -o json
  lightning node metrics get --engine-url http://localhost:8080`,
...
func runNodeMetricsGet(cmd *cobra.Command, args []string) error {
	...
	if config.GetCurrentNodeID() == "" {
		return fmt.Errorf("engine URL not set. Use 'lightning node context set <node-id>' first")
	}
...
Examples:
  ...
  lightning node module list -o json`,
...
Use 'lightning node module reload' to reinstall from disk.

Examples:
  lightning node module uninstall RigidBodyModule`,
	Args: cobra.ExactArgs(1),
...
When proxy is enabled (default), Lightning CLI routes requests to engine nodes
...
When enabled, all node requests are routed through the control plane proxy:
  Lightning CLI -> Control Plane -> Node
...
When disabled, Lightning CLI connects directly to node URLs. This requires
...
Examples:
  ...
  lightning node proxy status -o json`,
