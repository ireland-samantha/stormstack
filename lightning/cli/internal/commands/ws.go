...
import (
	...
	"github.com/ireland-samantha/stormstack/lightning-cli/internal/ws"
...
Before using ws commands, you must join a match using 'lightning match join'.
...
which is set when you join a match using 'lightning match join'.

Examples:
  lightning ws connect command    # Connect to command WebSocket
  lightning ws connect snapshot   # Connect to snapshot WebSocket`,
...
func runWsConnect(cmd *cobra.Command, args []string) error {
	...
	if wsClient != nil && wsClient.IsConnected() {
		return fmt.Errorf("already connected to %s. Use 'lightning ws disconnect' first", wsClient.URL())
	}
	...
	if wsURL == "" {
		return fmt.Errorf("no %s WebSocket URL configured. Join a match first with 'lightning match join'", wsType)
	}

	token := config.GetCurrentMatchToken()
	if token == "" {
		return fmt.Errorf("no match token configured. Join a match first with 'lightning match join'")
	}
...
Examples:
  lightning ws send '{"command":"move","x":10,"y":20}'
  lightning ws send --binary "SGVsbG8gV29ybGQ="`,
	Args: cobra.ExactArgs(1),
	RunE: runWsSend,
}
...
func runWsSend(cmd *cobra.Command, args []string) error {
	if wsClient == nil || !wsClient.IsConnected() {
		return fmt.Errorf("not connected. Use 'lightning ws connect' first")
	}
...
Examples:
  lightning ws receive                     # Receive one message
  lightning ws receive --timeout 10s       # Wait up to 10 seconds
  lightning ws receive --count 5           # Receive 5 messages
  lightning ws receive -c 0                # Receive continuously (Ctrl+C to stop)`,
	RunE: runWsReceive,
}
