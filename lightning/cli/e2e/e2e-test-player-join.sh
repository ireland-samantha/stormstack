...
#
# This script tests the full player match join flow using Lightning CLI:
...
# - Lightning CLI binary available (./thunder or in PATH)
#
# Usage:
...
# Lightning CLI - try local binary first, then PATH
if [ -x "$SCRIPT_DIR/../thunder" ]; then
    THUNDER_CMD="$SCRIPT_DIR/../thunder"
elif command -v thunder &> /dev/null; then
    THUNDER_CMD="thunder"
else
    echo "ERROR: thunder CLI not found. Build it with: cd thunder-cli && go build -o lightning ./cmd/lightning"
    exit 1
fi
...
# =============================================================================
# Step 2: Configure Lightning CLI
# =============================================================================

log_step "Step 2: Configuring Lightning CLI..."

$THUNDER_CMD config set control_plane_url "$CONTROL_PLANE_URL"

log_info "Lightning CLI configured"

# =============================================================================
# Step 3: Authenticate with Lightning CLI
# =============================================================================

log_step "Step 3: Authenticating with Lightning CLI (control plane auth proxy)..."
...
if [ -f ~/.lightning.yaml ]; then
    # Fix Docker hostname to localhost
    sed -i.bak 's|ws://backend:8080|ws://localhost:8080|g' ~/.lightning.yaml
    # Fix command WebSocket path (/ws/containers -> /containers)
    sed -i.bak 's|/ws/containers/\([0-9]*\)/commands|/containers/\1/commands|g' ~/.lightning.yaml
    # Fix snapshot WebSocket path (snapshots -> snapshot, singular)
    sed -i.bak 's|/snapshots$|/snapshot|g' ~/.lightning.yaml
...
# Note: Lightning CLI WebSocket commands require a persistent connection daemon
# which is not yet implemented. For now, verify the URLs are correctly formed
# and available in the config.

COMMAND_WS_CONFIG=$(grep "current_command_ws_url" ~/.lightning.yaml | cut -d' ' -f2)
SNAPSHOT_WS_CONFIG=$(grep "current_snapshot_ws_url" ~/.lightning.yaml | cut -d' ' -f2)
...
echo "  3. Lightning CLI configured         - PASSED"
