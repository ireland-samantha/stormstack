...
Examples:
  thunder match list
  thunder match list --status RUNNING
  thunder match list -o json`,
	RunE: runMatchList,
}
...
Examples:
  thunder match get node-1-42-7
  thunder match get node-1-42-7 -o json`,
...
Examples:
  thunder match join node-1-42-7 --player-name "Player1" --player-id "player-123"
  thunder match join node-1-42-7 -n "Alice" -p "alice-001" -o json`,
	Args: cobra.ExactArgs(1),
	RunE: runMatchJoin,
}
...
func runMatchJoin(cmd *cobra.Command, args []string) error {
	...
	out.PrintMessage("\nUse 'thunder ws connect command' or 'thunder ws connect snapshot' to connect.")
	return nil
}
...
Examples:
  thunder match finish node-1-42-7`,
	Args: cobra.ExactArgs(1),
	RunE: runMatchFinish,
}
...
Examples:
  thunder match delete node-1-42-7`,
	Args: cobra.ExactArgs(1),
	RunE: runMatchDelete,
}
...
// context set subcommand (deprecated - use 'thunder node context set')
var matchContextSetCmd = &cobra.Command{
	Use:        "set <match-id>",
	Short:      "Set the current match context (deprecated: use 'thunder node context set')",
	Deprecated: "use 'thunder node context set <node-id>' and 'thunder node context match <match-id>' instead",
	Args:       cobra.ExactArgs(1),
...
func runMatchContextSet(cmd *cobra.Command, args []string) error {
	...
	out.PrintMessage("Note: 'thunder match context set' is deprecated. Use 'thunder node context set <node-id>' instead.")
	return nil
}
...
// context show subcommand (deprecated - use 'thunder node context show')
var matchContextShowCmd = &cobra.Command{
	Use:        "show",
	Short:      "Show the current match context (deprecated: use 'thunder node context show')",
	Deprecated: "use 'thunder node context show' instead",
