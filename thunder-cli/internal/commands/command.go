...
Use 'thunder command list' to see available commands and their parameters.

Examples:
  thunder command send spawn '{"matchId":1,"playerId":1,"entityType":100}'
  thunder command send move '{"entityId":1,"x":10,"y":20}'
  thunder command send tick`,
...
Examples:
  thunder command send-bulk commands.json
  thunder command send-bulk commands.json --advance-tick`,
...
Examples:
  thunder command list
  thunder command list -o json`,
	RunE: runCommandList,
}
