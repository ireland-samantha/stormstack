#!/bin/bash
#
# Thunder CLI End-to-End Test Script
#
# This script tests the full Thunder CLI workflow:
#   1. Builds the Thunder CLI binary
...
#   ./e2e-test-physics.sh --skip-build           # Skip building Thunder CLI
...
AUTH_STATUS=$(run_thunder auth status 2>&1)
if [[ "$AUTH_STATUS" != *"Authenticated"* ]]; then
    log_error "Thunder CLI authentication failed. Status: $AUTH_STATUS"
    exit 1
fi
...
for i in $(seq 1 $NUM_DEPLOYMENTS); do
    ...
    DEPLOY_OUTPUT=$(run_thunder deploy --modules "$TEST_MODULES" -o json) || true
...
for idx in "${!MATCH_IDS[@]}"; do
    ...
    TICK_OUTPUT=$(run_thunder node tick advance -o json)
...
for idx in "${!MATCH_IDS[@]}"; do
    ...
    # Start auto-advance
    PLAY_OUTPUT=$(run_thunder node simulation play --interval-ms "$TICK_INTERVAL_MS" 2>&1)
...
run_thunder node context set "${NODE_IDS[0]}" --container-id "${CONTAINER_IDS[0]}" --match-id "${MATCH_IDS[0]}" > /dev/null 2>&1

log_info "Node Metrics:"
NODE_METRICS=$(run_thunder node metrics get -o json)
...
for idx in "${!MATCH_IDS[@]}"; do
    ...
    run_thunder node context set "$NODE_ID" --container-id "$CONTAINER_ID" --match-id "$MATCH_ID" > /dev/null 2>&1
    METRICS=$(run_thunder node metrics container -o json)
...
CLUSTER_STATUS=$(run_thunder cluster status -o json)
...
echo ""
echo "Test Summary:"
echo "  - Thunder CLI built and verified"
