#!/bin/bash
#
# Thunder CLI Module Management E2E Test Script
#
# This script tests module uninstall and reinstall operations using Thunder CLI:
#   1. Deploys a game with RigidBodyModule
#   2. Verifies RigidBodyModule is active via 'run_thunder node module list'
#   3. Spawns entities and runs simulation
#   4. Uninstalls RigidBodyModule via 'run_thunder node module uninstall'
#   5. Verifies module is uninstalled
#   6. Reloads modules via 'run_thunder node module reload'
#   7. Verifies RigidBodyModule is reinstalled and working
#
# Usage:
#   ./e2e-test-modules.sh              # Run full test
#   ./e2e-test-modules.sh --skip-build # Skip building Thunder CLI
...
THUNDER_CLI_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
THUNDER_BIN="$THUNDER_CLI_DIR/thunder"
THUNDER_CONFIG="$HOME/.thunder.yaml"
...
log_header() { echo -e "\n${BOLD}${CYAN}=== $1 ===${NC}\n"; }
log_cmd() { echo -e "${CYAN}[CMD]${NC} thunder $*" >&2; }

# Helper to run thunder commands with logging
run_thunder() {
    log_cmd "$@"
    "$THUNDER_BIN" "$@"
}
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo ""
            echo "Options:"
            echo "  --skip-build    Skip building Thunder CLI"
...
cleanup() {
    ...
    if [ -n "$MATCH_ID" ]; then
        ...
        run_thunder deploy undeploy "$MATCH_ID" > /dev/null 2>&1 || true
    fi

    # Remove thunder config if we created it
    if [ -f "$THUNDER_CONFIG.e2e-backup" ]; then
        mv "$THUNDER_CONFIG.e2e-backup" "$THUNDER_CONFIG"
        log_info "Restored original Thunder config"
    elif [ -f "$THUNDER_CONFIG" ]; then
        rm -f "$THUNDER_CONFIG"
    fi
...
# =============================================================================
# Step 1: Build Thunder CLI and Rigid Body Module
# =============================================================================
if [ "$SKIP_BUILD" = false ]; then
    log_info "Building Thunder CLI..."
    cd "$THUNDER_CLI_DIR"
    ...
    go build -o thunder ./cmd/thunder

    if [ ! -f "$THUNDER_BIN" ]; then
        log_error "Failed to build Thunder CLI"
        exit 1
    fi

    log_success "Thunder CLI built successfully"

    # Build rigid body module JAR
    log_info "Building Rigid Body Module JAR..."
    cd "$PROJECT_ROOT"
    mvn install -pl lightning-engine-extensions/modules/physics-module/rigid-body-module -DskipTests -q

    RIGID_BODY_JAR="$PROJECT_ROOT/lightning-engine-extensions/modules/physics-module/rigid-body-module/target/rigid-body-module-0.0.3-SNAPSHOT.jar"
    ...
    log_info "Skipping Thunder CLI build"
    if [ ! -f "$THUNDER_BIN" ]; then
        log_error "Thunder binary not found at $THUNDER_BIN. Run without --skip-build"
        exit 1
    fi
...
log_info "Verifying Thunder CLI..."
log_cmd "version"
if ! "$THUNDER_BIN" version &> /dev/null; then
    log_error "Thunder CLI failed to execute"
    exit 1
fi
log_success "Thunder CLI is working"
...
if [ -f "$THUNDER_CONFIG" ]; then
    cp "$THUNDER_CONFIG" "$THUNDER_CONFIG.e2e-backup"
fi

# Configure control plane URL first (needed for auth proxy)
run_thunder config set control_plane_url "$CONTROL_PLANE_URL"
...
if [ -n "$THUNDER_API_TOKEN" ]; then
    ...
    run_thunder auth token "$THUNDER_API_TOKEN"
else
    ...
    if ! run_thunder auth login --username "$TEST_USERNAME" --password "$TEST_PASSWORD"; then
...
log_success "Thunder CLI authenticated successfully"

# Verify proxy mode status (should be enabled by default)
log_info "Checking proxy mode status..."
PROXY_STATUS=$(run_thunder node proxy status -o json)
...
if [ -n "$RIGID_BODY_JAR" ] && [ -f "$RIGID_BODY_JAR" ]; then
    ...
    UPLOAD_OUTPUT=$(run_thunder module upload "$TARGET_MODULE" "0.0.3-SNAPSHOT" "$RIGID_BODY_JAR" --description "Physics rigid body simulation" 2>&1)
    ...
    DISTRIBUTE_OUTPUT=$(run_thunder module distribute "$TARGET_MODULE" "0.0.3-SNAPSHOT" 2>&1)
...
DEPLOY_OUTPUT=$(run_thunder deploy --modules "$TEST_MODULES" -o json)
...
# =============================================================================
# Step 5b: Verify deployment using run_thunder node list and match list
# =============================================================================
log_header "Verifying Deployment with List Commands"

log_info "Listing cluster nodes..."
NODE_LIST=$(run_thunder node list -o json)
...
echo ""
log_info "Listing matches with 'run_thunder match list'..."
MATCH_LIST=$(run_thunder match list -o json)
...
run_thunder node context set "$NODE_ID" --container-id "$CONTAINER_ID" --match-id "$MATCH_ID" > /dev/null 2>&1

# =============================================================================
# Step 6: Verify module is installed using Thunder CLI
# =============================================================================
log_header "Verifying Module Installation"

log_info "Running: run_thunder node module list"
MODULE_LIST=$(run_thunder node module list -o json)
...
for i in $(seq 1 5); do
    ...
    run_thunder command send spawn "$SPAWN_PARAMS" > /dev/null 2>&1
done

# Advance tick to process spawns
run_thunder node tick advance > /dev/null 2>&1

# Get initial snapshot using Thunder CLI
SNAPSHOT_BEFORE=$(run_thunder snapshot get -o json)
...
COMMANDS_FILE="/tmp/thunder-module-commands.json"
...
BULK_OUTPUT=$(run_thunder command send-bulk "$COMMANDS_FILE" 2>&1)
...
run_thunder node simulation play --interval-ms 16 > /dev/null 2>&1

sleep 2

run_thunder node simulation stop > /dev/null 2>&1

# Get snapshot after physics using Thunder CLI
SNAPSHOT_AFTER=$(run_thunder snapshot get -o json)
...
# =============================================================================
# Step 8: Uninstall RigidBodyModule using Thunder CLI
# =============================================================================
log_header "Uninstalling $TARGET_MODULE"

log_info "Running: run_thunder node module uninstall $TARGET_MODULE"

UNINSTALL_OUTPUT=$(run_thunder node module uninstall "$TARGET_MODULE" 2>&1)
...
# =============================================================================
# Step 9: Verify module is uninstalled using Thunder CLI
# =============================================================================
log_header "Verifying Module Uninstallation"

log_info "Running: run_thunder node module list"
MODULES_AFTER_UNINSTALL=$(run_thunder node module list -o json)
...
TICK_OUTPUT=$(run_thunder node tick get -o json)
...
# =============================================================================
# Step 10: Reload modules to reinstall RigidBodyModule using Thunder CLI
# =============================================================================
log_header "Reinstalling $TARGET_MODULE via Reload"

log_info "Running: run_thunder node module reload"

RELOAD_OUTPUT=$(run_thunder node module reload 2>&1)
...
# =============================================================================
# Step 11: Verify module is reinstalled using Thunder CLI
# =============================================================================
log_header "Verifying Module Reinstallation"

log_info "Running: run_thunder node module list"
MODULES_AFTER_RELOAD=$(run_thunder node module list -o json)
...
FINAL_SNAPSHOT=$(run_thunder snapshot get -o json)
...
echo "  2. Verified module installed via 'run_thunder node module list'"
echo "  3. Ran physics simulation (entities moved)"
echo "  4. Uninstalled module via 'run_thunder node module uninstall $TARGET_MODULE'"
echo "  5. Verified module removed via 'run_thunder node module list'"
echo "  6. Ran ticks without physics (module unloaded)"
echo "  7. Reinstalled via 'run_thunder node module reload'"
echo "  8. Verified module reinstalled via 'run_thunder node module list'"
