...
public class PWSidebarNavigator {
    ...
    private void expandControlPlaneMenu() {
        // Click "Control Plane" to expand if not already expanded
        Locator controlPlaneButton = getMenuButton("Control Plane").first();
        // Check if already expanded by looking for menu items
        if (!getMenuButton("Nodes").isVisible()) {
            controlPlaneButton.click();
        }
    }

    private void expandEngineMenu() {
        // Click "Engine" section to expand if not already expanded
        Locator engineButton = getMenuButton("Engine").first();
    ...
    private void expandContainerMenu() {
        // Alias for expandEngineMenu for backward compatibility
        expandEngineMenu();
    }
    ...
    private void expandAuthMenu() {
        Locator authButton = getMenuButton("Authentication");
    ...
    private void expandIamMenu() {
        // Alias for expandAuthMenu for backward compatibility
        expandAuthMenu();
    }

    // ==================== Control Plane Navigation ====================

    public PWClusterOverviewPage goToClusterOverview() {
        expandControlPlaneMenu();
        getMenuButton("Overview").click();
        page.waitForLoadState();
        return new PWClusterOverviewPage(page);
    }

    public PWClusterNodesPage goToClusterNodes() {
        expandControlPlaneMenu();
        getMenuButton("Nodes").click();
        page.waitForLoadState();
        return new PWClusterNodesPage(page);
    }

    public PWClusterModulesPage goToClusterModules() {
        expandControlPlaneMenu();
        getMenuButton("Modules").click();
        page.waitForLoadState();
        return new PWClusterModulesPage(page);
    }

    public PWDeploymentsPage goToDeployments() {
        expandControlPlaneMenu();
        getMenuButton("Deployments").click();
        page.waitForLoadState();
        return new PWDeploymentsPage(page);
    }

    public PWAutoscalerPage goToAutoscaler() {
        expandControlPlaneMenu();
        getMenuButton("Autoscaler").click();
        page.waitForLoadState();
        return new PWAutoscalerPage(page);
    }
