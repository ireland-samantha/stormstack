...
import {
    Add as AddIcon,
    Delete as DeleteIcon, Dns as ContainerIcon, Extension as ModulesIcon, Refresh as RefreshIcon,
    SportsEsports as MatchIcon
} from "@mui/icons-material";
import {
    Alert, Autocomplete, Avatar, Box, Button, Card,
    CardContent, Chip,
    CircularProgress, Dialog, DialogActions, DialogContent, DialogTitle, Grid, IconButton, Paper, Stack, TextField, Tooltip, Typography
} from "@mui/material";
import { useState } from "react";
import {
    useCreateMatchInContainerMutation,
    useDeleteMatchFromContainerMutation, useGetContainerMatchesQuery, useGetContainerQuery, useGetModulesQuery
...
const MatchesPanel: React.FC = () => {
  ...
  const [createMatch, { isLoading: isCreating }] =
    useCreateMatchInContainerMutation();
  const [deleteMatch, { isLoading: isDeleting }] =
    ...
    <Box>
      ...
      {selectedContainer.status !== "RUNNING" && (
        <Alert severity="warning" sx={{ mb: 2 }}>
          Container is not running. Start the container to create matches and
          interact with them.
        </Alert>
      )}
            ...
            <Grid size={12}>
              <Paper sx={{ p: 4, textAlign: "center" }}>
                ...
                <Typography color="text.secondary" sx={{ mb: 2 }}>
                  {selectedContainer.status === "RUNNING"
                    ? "Create a match to start running game simulations"
                    : "Start the container first, then create matches"}
                </Typography>
                {selectedContainer.status === "RUNNING" && (
                  <Button
                    variant="contained"
      ...
      >
        ...
        <DialogContent>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
            Select which modules to enable for this match. Modules provide
      ...
      >
        ...
        <DialogContent>
          <Typography>
            Are you sure you want to delete Match {deletingMatchId}? This action
            cannot be undone.
          </Typography>
        </DialogContent>
        <DialogActions>
