...
class SnapshotPanelTest {
    ...
    void setSnapshotData_createsTreeWithMatchAndModules() {
        ...
        List<String> moduleNames = matchNode.getChildren().stream()
            ...
            .toList();
        assertThat(moduleNames).containsExactlyInAnyOrder("EntityModule", "MoveModule");
    }
    ...
    void setSnapshotData_createsEntityNodesWithCorrectNames() {
        ...
        // Find EntityModule
        TreeNode entityModule = matchNode.getChildren().stream()
            .filter(n -> n.getLabel().equals("EntityModule"))
            .findFirst()
            .orElseThrow();

        // Verify entity node names (using ENTITY_ID values from test data)
        List<String> entityNames = entityModule.getChildren().stream()
            .map(TreeNode::getLabel)
            .toList();
    ...
    void setSnapshotData_entityNodesHaveNoChildrenInTree() {
        ...
        TreeNode entity0 = entityModule.getChildren().get(0);
    ...
    void setSnapshotData_preservesModuleExpansionState() {
        ...
        // Collapse EntityModule
        entityModule.setExpanded(false);
        ...
        entityModule = matchNode.getChildren().stream()
            .filter(n -> n.getLabel().equals("EntityModule"))
            .findFirst()
            .orElseThrow();

        assertThat(entityModule.isExpanded()).isFalse();
    }
    ...
    void setSnapshotData_multipleEntitiesShowInTree() {
        ...
        TreeNode rootNode = tree.getRootNodes().get(0);
        TreeNode matchNode = rootNode.getChildren().get(0);
        TreeNode entityModuleNode = matchNode.getChildren().stream()
            .filter(n -> n.getLabel().equals("EntityModule"))
            .findFirst()
            .orElseThrow();

        // Check all entities are listed (using ENTITY_ID from test data)
        assertThat(entityModuleNode.getChildren()).hasSize(3);
        assertThat(entityModuleNode.getChildren().get(0).getLabel()).isEqualTo("Entity 101");
        assertThat(entityModuleNode.getChildren().get(1).getLabel()).isEqualTo("Entity 102");
        assertThat(entityModuleNode.getChildren().get(2).getLabel()).isEqualTo("Entity 103");

        // Entity nodes should have no children (components shown in detail panel)
        for (TreeNode entityNode : entityModuleNode.getChildren()) {
    ...
    private SnapshotData createTestSnapshot() {
        ...
        // EntityModule with 3 entities
        Map<String, List<Float>> entityData = new HashMap<>();
        entityData.put("ENTITY_ID", List.of(101.0f, 102.0f, 103.0f));
        entityData.put("ENTITY_TYPE", List.of(1.0f, 2.0f, 3.0f));
        entityData.put("OWNER_ID", List.of(100.0f, 100.0f, 200.0f));
        entityData.put("POSITION_X", List.of(50.0f, 100.0f, 150.0f));
        entityData.put("POSITION_Y", List.of(60.0f, 110.0f, 160.0f));
        data.put("EntityModule", entityData);
