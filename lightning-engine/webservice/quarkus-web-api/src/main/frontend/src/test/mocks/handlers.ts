...
const mockContainers = [
  { id: 1, name: 'default', status: 'RUNNING' as const, currentTick: 100, autoAdvancing: false, autoAdvanceIntervalMs: 16 },
  { id: 2, name: 'test', status: 'STOPPED' as const, currentTick: 0, autoAdvancing: false, autoAdvanceIntervalMs: 16 },
];

const mockMatches = [
  { id: 1, name: 'Match 1', status: 'active', tick: 50, modules: ['EntityModule', 'RigidBodyModule'] },
  { id: 2, name: 'Match 2', status: 'active', tick: 25, modules: ['EntityModule'] },
];
...
const mockRoles = [
  { id: 1, name: 'admin', description: 'Full access', includedRoles: ['view_only', 'command_manager'] },
  { id: 2, name: 'view_only', description: 'Read-only access', includedRoles: [] },
  { id: 3, name: 'command_manager', description: 'Can execute commands', includedRoles: ['view_only'] },
];
...
const mockCommands = [
  { name: 'spawn', description: 'Spawn an entity', module: 'EntityModule', parameters: [
    { name: 'type', type: 'number', required: true, description: 'Entity type' },
    { name: 'x', type: 'number', required: false, description: 'X position' },
    { name: 'y', type: 'number', required: false, description: 'Y position' },
  ]},
  { name: 'move', description: 'Move an entity', module: 'RigidBodyModule', parameters: [
...
const mockModules = [
  { name: 'EntityModule', version: '1.0.0', description: 'Core entity management', components: ['ENTITY_ID', 'OWNER'], source: 'builtin' as const },
  { name: 'RigidBodyModule', version: '1.0.0', description: 'Physics simulation', components: ['POSITION_X', 'POSITION_Y', 'VELOCITY_X', 'VELOCITY_Y'], source: 'builtin' as const },
];

const mockAIs = [
  { name: 'TickCounter', version: '1.0.0', description: 'Counts ticks', requiredModules: [], source: 'builtin' as const },
];

const mockResources = [
  { id: 'res-1', name: 'sprite.png', contentType: 'image/png', mimeType: 'image/png', size: 1024, createdAt: '2026-01-01T00:00:00Z' },
  { id: 'res-2', name: 'sound.wav', contentType: 'audio/wav', mimeType: 'audio/wav', size: 2048, createdAt: '2026-01-02T00:00:00Z' },
];
...
export const handlers = [
  ...
  http.post('/api/auth/login', async ({ request }) => {
    ...
    if (body.username === 'admin' && body.password === 'admin') {
      return HttpResponse.json({ token: 'mock-jwt-token', refreshToken: 'mock-refresh-token' });
    }
  ...
  http.get('/api/auth/me', async () => {
    await delay(50);
    return HttpResponse.json({ id: 1, username: 'admin', roles: ['admin'], enabled: true });
  }),
  ...
  http.post('/api/containers/:containerId/matches', async ({ request }) => {
    await delay(100);
    const body = await request.json() as { id?: number; enabledModuleNames?: string[] };
    const newMatch = {
  ...
  http.post('/api/auth/users', async ({ request }) => {
    await delay(100);
    const body = await request.json() as { username: string; password: string; roles?: string[] };
    const newUser = {
  ...
  http.post('/api/auth/roles', async ({ request }) => {
    await delay(100);
    const body = await request.json() as { name: string; description?: string; includes?: string[] };
    const newRole = {
  ...
  http.get('/api/containers/:containerId/matches/:matchId/players', async ({ params }) => {
    await delay(100);
  ...
  http.post('/api/containers/:containerId/matches/:matchId/sessions', async ({ request }) => {
    await delay(100);
  ...
  http.post('/api/containers/:containerId/matches/:matchId/sessions/:playerId/disconnect', async () => {
    await delay(50);
    return new HttpResponse(null, { status: 204 });
  }),

  http.post('/api/containers/:containerId/matches/:matchId/sessions/:playerId/reconnect', async ({ params }) => {
    await delay(50);
    return HttpResponse.json({
      id: 1,
  ...
  http.post('/api/containers/:containerId/matches/:matchId/sessions/:playerId/abandon', async () => {
    await delay(50);
    return new HttpResponse(null, { status: 204 });
  }),
  ...
  http.post('/api/containers/:containerId/modules/reload', async () => {
    await delay(200);
    return HttpResponse.json(['EntityModule', 'RigidBodyModule', 'RenderModule']);
  }),
  ...
  http.delete('/api/containers/:containerId/resources/:resourceId', async () => {
    await delay(50);
    return new HttpResponse(null, { status: 204 });
  }),
