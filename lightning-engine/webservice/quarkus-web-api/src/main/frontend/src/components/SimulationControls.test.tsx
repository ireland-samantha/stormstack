...
describe('SimulationControls', () => {
    ...
    server.use(
      http.get('*/api/containers', () => {
        return HttpResponse.json([
          { id: 1, name: 'test', status: 'RUNNING', currentTick: 0, autoAdvancing: false }
        ]);
      }),
  ...
  it('renders simulation buttons when container is selected', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByRole('button', { name: /advance one tick/i })).toBeInTheDocument();
    });
  ...
  it('shows stop button when simulation is playing', async () => {
    ...
    expect(screen.getByRole('button', { name: /advance one tick/i })).toBeDisabled();
  });
  ...
  it('calls tick endpoint when tick button is clicked', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByRole('button', { name: /advance one tick/i })).toBeEnabled();
    });
  ...
  it('calls stop-auto endpoint when stop button is clicked', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByRole('button', { name: /stop simulation/i })).toBeEnabled();
    });
