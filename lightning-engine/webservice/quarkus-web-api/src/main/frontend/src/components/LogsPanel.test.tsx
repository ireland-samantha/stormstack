...
vi.mock('../services/api', async (importOriginal) => {
  ...
  return {
    ...actual,
    buildPlayerErrorWebSocketUrl: vi.fn((matchId, playerId) =>
...
describe('LogsPanel', () => {
  it('renders panel after loading', async () => {
    renderWithProviders(<LogsPanel />, {
      preloadedState: {
        ui: { selectedContainerId: 1, selectedMatchId: null, activePanel: 'logs' }
      }
    });
  ...
  it('shows no container selected message when container is null', async () => {
    renderWithProviders(<LogsPanel />, {
      preloadedState: {
        ui: { selectedContainerId: null, selectedMatchId: null, activePanel: 'logs' }
      }
    });
