...
describe('ContainerDashboard', () => {
  ...
  it('renders loading state when fetching containers', () => {
    ...
    expect(screen.queryAllByRole('progressbar').length).toBeGreaterThanOrEqual(0);
  });
  ...
  it('renders container cards when containers exist', async () => {
    server.use(
      http.get('*/api/containers', () => {
        return HttpResponse.json([
          { id: 1, name: 'game-server-1', status: 'RUNNING', currentTick: 10, matchCount: 2 },
          { id: 2, name: 'test-server', status: 'STOPPED', currentTick: 0, matchCount: 0 }
        ]);
      })
    );
  ...
  it('selects container when card is clicked', async () => {
    ...
    server.use(
      ...
      http.get('*/api/containers/1', () => {
        return HttpResponse.json({ id: 1, name: 'game-server-1', status: 'RUNNING', currentTick: 10 });
      }),
