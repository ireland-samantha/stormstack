...
describe('CommandsPanel', () => {
  it('renders the panel title', async () => {
    renderWithProviders(<CommandsPanel />, {
      preloadedState: {
        ui: { selectedContainerId: 1, selectedMatchId: null, activePanel: 'commands' }
      }
    });
  ...
  it('shows no container selected message when container is null', async () => {
    renderWithProviders(<CommandsPanel />, {
      preloadedState: {
        ui: { selectedContainerId: null, selectedMatchId: null, activePanel: 'commands' }
      }
    });
  ...
  it('has tabs for available commands and quick send', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByRole('tab', { name: /available commands/i })).toBeInTheDocument();
      expect(screen.getByRole('tab', { name: /quick send/i })).toBeInTheDocument();
    });
  });
