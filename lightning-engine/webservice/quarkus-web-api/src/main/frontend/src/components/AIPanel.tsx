...
import {
  Refresh as RefreshIcon, Psychology as AIIcon, Info as InfoIcon
...
const AIPanel: React.FC = () => {
  ...
  const { data: aiNames = [], isLoading, error: fetchError, refetch } = useGetContainerAIQuery(
    selectedContainerId!,
  ...
  return (
    <Box>
            ...
            {aiNames.length === 0 && (
              <TableRow>
                <TableCell colSpan={2} align="center">
                  <Typography color="text.secondary">No AI installed in this container</Typography>
                </TableCell>
              </TableRow>
            )}
          </TableBody>
      ...
      {aiNames.length > 0 && (
        <Box sx={{ mt: 3 }}>
          <Typography variant="h6" gutterBottom>Available AI ({aiNames.length})</Typography>
      ...
      <Paper sx={{ mt: 3, p: 2 }}>
        <Typography variant="subtitle1" gutterBottom>About AI</Typography>
        <Typography variant="body2" color="text.secondary">
          AIs are autonomous controllers that can manage game logic, spawn entities,
          and execute commands automatically during simulation. Each container has its own
          set of available AI that can be enabled when creating matches.
        </Typography>
      </Paper>
    </Box>
  );
