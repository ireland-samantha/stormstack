...
describe('AIPanel', () => {
  ...
  it('renders no container selected message when no container is selected', () => {
    ...
    expect(screen.getByText(/select a container from the sidebar/i)).toBeInTheDocument();
  });
  ...
  it('renders empty state when no AI installed', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/no ai installed in this container/i)).toBeInTheDocument();
    });
  });
