...
import {
  ...
  Extension as ModuleIcon, Info as InfoIcon
...
const ModulesPanel: React.FC = () => {
  ...
  const { data: moduleNames = [], isLoading, error: fetchError, refetch } = useGetContainerModulesQuery(
    selectedContainerId!,
  ...
  const handleReloadModules = async () => {
    ...
    try {
      ...
      setLocalError(err instanceof Error ? err.message : 'Failed to reload modules');
    } finally {
      setReloading(false);
    }
  };
  ...
  return (
    <Box>
            ...
            {moduleNames.length === 0 && (
              <TableRow>
                <TableCell colSpan={2} align="center">
                  <Typography color="text.secondary">No modules installed in this container</Typography>
                </TableCell>
              </TableRow>
            )}
          </TableBody>
      ...
      {moduleNames.length > 0 && (
        <Box sx={{ mt: 3 }}>
          <Typography variant="h6" gutterBottom>Installed Modules ({moduleNames.length})</Typography>
